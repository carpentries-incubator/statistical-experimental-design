...
```{r load_data, message=FALSE, eval=FALSE}
...
`r round((mean(controlHR) - mean(highHR))/mean(controlHR)* 100, 1)` 
...
`r sum(population$heart_rate > 80)` individuals 
(~`r round((sum(population$heart_rate > 80)/dim(population)[1]) * 100, 1)`%) 
