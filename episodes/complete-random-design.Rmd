...
heart_rate %>% mutate(exercise_group = fct_reorder(exercise_group, 
  ...
  geom_segment(x = 1.15, y = 61.6, xend = 1.15, yend = 63.5) +
  geom_segment(x = 1.02, y = 61.7, xend = 1.15, yend = 61.7) +
  geom_segment(x = 1, y = 63.5, xend = 1.15, yend = 63.5)
```
...
`r as.integer(anova(lm(heart_rate ~ exercise_group, data = heart_rate))[[2]][2])`.
...
= 
`r anova(lm(heart_rate ~ exercise_group, data = heart_rate))[[3]][1]`
). 
...
(`r anova(lm(heart_rate ~ exercise_group, data = heart_rate))[[3]][1]` /
`r round(anova(lm(heart_rate ~ exercise_group, data = heart_rate))[[3]][2], 1)` = 
...
(`r round(lm(heart_rate ~ exercise_group + age, data = heart_rate)$coef[[4]], 2)`)
and has a high p-value 
(`r summary(lm(heart_rate ~ exercise_group + age, data = heart_rate))$coef[4, 4]`).
...
`r round(lm(heart_rate ~ exercise_group + age, data = heart_rate)$coef[[1]], 1)`
...
data <- data.frame(dose=c(6, 9, 12, 14, 30, 35, 40, 47, 51, 55, 60),
                   weight=c(14, 28, 50, 70, 89, 94, 90, 75, 59, 44, 27))
data %>% ggplot(aes(dose, weight)) + 
  geom_point() + 
