...
heat_data <- drugExercise %>%
  ...
  spread(DrugDose, meanChange)
heat_matrix <- as.matrix(heat_data[,-1])
rownames(heat_matrix) <- heat_data$Exercise
heatmap(heat_matrix, Rowv = NA, Colv = NA, col = heat.colors(10),
        main = "Heatmap of Mean Δ Glucose",
        xlab = "Drug Dosage (mg/kg)", ylab = "Exercise (min/day)")
```
...
knitr::kable(pivot_wider(meansSD, 
                         names_from = DrugDose, 
                         values_from = stDev),
             caption = "Standard deviation in glucose by group") %>% 
  add_header_above(c(" " = 2, "Drug Dose" = 3))
```
...
at
`r round(anova(lm(Delta ~ DrugDose*Exercise, data = drugExercise))[2, 5], 3)`
