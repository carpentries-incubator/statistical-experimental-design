...
text_tbl <- data.frame('source of variation' = c('treatment 1', 
                                                 'treatment 2',
                                                 'interaction',
                                                 'error'),
                       'Df' = c("$k_1 - 1$",
...
kable(text_tbl,
      caption = 'ANOVA Table for Completely Randomized Design with Two or More Treatment Factors') 
```
...
```{r meansSDinteractionplotExercise}
ggplot(meansSD, aes(x=Exercise, y=meanChange, group=DrugDose, color=DrugDose)) + 
    geom_line() +
    geom_point() +
    geom_errorbar(aes(ymin=meanChange-stDev, ymax=meanChange+stDev), width=.2,
