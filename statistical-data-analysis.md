...
Increasing power reduces the false negative rate ($\beta$) and increases the 
false positive rate ($\alpha$). The tradeoff depends on the consequences of a 
false positive versus those for a false negative. A false positive might lead
to a paper retraction, while a false negative might mean missing an opportunity
to publish an interesting result. Generally the consequences of false positives
outweight those of false negatives, so $\alpha$ is set much lower than $\beta$.
